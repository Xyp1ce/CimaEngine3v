...
void Camara::setViewSize(float x, float y) { m_view->setSize({x, y}); }
...
void CamaraCuadro::onUpdate(float dt) {
  ...
  std::string log =
      "ObJ: (" + std::to_string(opos.x) + "," + std::to_string(opos.y) + ")";
...
void CamaraLERP::onUpdate(float dt) {
  ...
  GLogger::Get().agregarLog("(" + std::to_string(m_transform->posicion.x) +
                                "," + std::to_string(m_transform->posicion.y) +
                                ")",
...
// Camara SnapVentana
CamaraSnapVentana::CamaraSnapVentana(const Vector2D &pos, const Vector2D &dim,
                                     const Vector2D &vdim)
...
CamaraSnapMario::CamaraSnapMario(const Vector2D &pos, const Vector2D &dim,
                                 const Vector2D &vdim)
    : Camara{pos, dim}, m_vdim{vdim} {}

void CamaraSnapMario::onUpdate(float dt) {
  Camara::onUpdate(dt);
  if (!m_lockObj.lock())
    return;
  if (!lock) {
  ...
  if (jpos.y + offset >= ldn) {
    ...
    m_transform->posicion.y += ((jpos.y - offset) - lup);
  }
}
} // namespace CE

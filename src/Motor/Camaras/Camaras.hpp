 ...
 *
 * Implementa una cámara que puede seguir objetos y está vinculada a un view de
 * SFML. Proporciona transformación (posición, rotación) y métodos de
 * actualización virtual para permitir diferentes estrategias de movimiento
 * (fija, seguimiento, LERP, etc).
 *
 * @note Utiliza weak_ptr para evitar referencias circulares con el objeto
 * seguido
 */
class Camara {
public:
   ...
   */
  [[nodiscard]] ITransform &getTransformada() { return *m_transform; }

  /**
   * @brief Obtiene la vista SFML de la cámara.
   * @return Referencia constante a sf::View
   */
  [[nodiscard]] sf::View &getView() const { return *m_view; }
 ...
 * @brief Cámara que sigue objetos después de que crucen un área (la ventana) de
 * lo contrario no la sigue. Bueno para juegos de plataforma
 *
 */
class CamaraSnapVentana : public Camara {
public:
  CamaraSnapVentana(const Vector2D &pos, const Vector2D &dim,
                    const Vector2D &dven);
...
class CamaraSnapMario : public Camara {
public:
  CamaraSnapMario(const Vector2D &pos, const Vector2D &dim,
                  const Vector2D &dven);
  ~CamaraSnapMario() override {};
  void onUpdate(float dt) override;

...
#include <Motor/GUI/GHistograma.hpp>
...
int Motor::OnRun(void) {
  ...
  while (mi_app->EstaCorriendo()) {
    ...
    ms = std::chrono::duration_cast<std::chrono::duration<float>>(frame_act -
                                                                  frame_ant)
             .count();
    frame_ant = frame_act;
  }
...
void Motor::OnInit(void) {
  ...
  GestorCamaras::Get().agregarCamara(
      std::make_shared<Camara>(Vector2D{540, 360}, Vector2D{1080, 720}));
  GestorCamaras::Get().setCamaraActiva(0);

  // font por defecto
  GestorAssets::Get().agregarFont("default_font",
                                  ASSETS "/fonts/Electrolize.ttf");

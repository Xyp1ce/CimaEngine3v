...
Juego::Juego() : CE::GameLayer{} { termino = false; }
void Juego::OnInit(void) {
  ...
  CE::GestorEscenas::Get().registrarEscena("Init",
...
void Juego::OnInputs(CE::InputBuffer &buffer) {
  ...
  while (!buffer.estaVacio()) {
    ...
    if (const auto *tecla_presionada = evento.getIf<sf::Event::KeyPressed>()) {
      ...
      procesado = true;
    } else if (const auto *tecla_liberada =
                   evento.getIf<sf::Event::KeyReleased>()) {
...
void Juego::OnRender() { escena_actual->onRender(); }

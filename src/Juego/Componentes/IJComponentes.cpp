...
IDialogo::IDialogo() : IInteractuable(), texto{L""}, id_texto{0} {
  texto = L"Texto de Prueba ABC ABC, se debe cargar por el arbol, al precionar "
          L"una tecla"
          L"se avanza al siguiente texto del dialogo, esto es solo una "
          L"demostración."
          L" El texto debe tener saltos de linea automáticos para no salirse "
          L"de la caja"
...
std::wstring IDialogo::agregarSaltoLinea(const std::wstring &str,
                                         size_t max_len) {
...
IIndicador::IIndicador(const sf::Texture &ref, float escala, Entidad *p,
                       const sf::Color &c)
  ...
  parent = p;
  if (!shader.loadFromFile(ASSETS "/shaders/prueba.frag",
...
void IIndicador::onInteractuar(CE::Objeto &obj) { (void)obj; }
void IIndicador::onRender() {
  if (!activo)
    return;
  auto pos = parent->getTransformada()->posicion;
  auto sd =
      parent->getComponente<CE::ISprite>()->m_sprite.getGlobalBounds().size;
